...
def animate(indx):
    ...
    print(values1)
    labels1 = [f"Освобожденные {int((df_rape['amnt_stop_investigation'].values[0] + df_rape['amnt_opravdonyh'].values[0] + df_rape['uslov_osuzhd'].values[0])/total *100)}%" 
               ,f"Лишенные свободы {int((df_rape['total'].values[0])/total *100)}%"]

    plt.suptitle(f'''Случаи с изнасилованием связанными 
статьями 120 и 121 на {int(df_rape['year'].values[0])} год''', size = 23, fontweight="bold")
    ax1.pie(values1,wedgeprops={'width':0.4})
...
свободы {int((df['forever_jail'].values[0])/ttl*100)}%''']
    ...
    ax2.pie(values2, wedgeprops={'width':0.4})
...
осужденные {int(df_rape['uslov_osuzhd'].values[0]/ttls * 100)}%''']
    ...
    ax3.pie(values3, wedgeprops={'width':0.4})
...
свободы: {int(df_rape['amnt_stop_investigation'].values[0] + df_rape['amnt_opravdonyh'].values[0] + df_rape['uslov_osuzhd'].values[0])}''', size = 17)

    df_total = df[df['ind'] <= indx]
    ax5.set_ylim(120, 260)
...
animation.save('violence.gif', dpi=100, writer=PillowWriter(fps=200)) # Script for saving
plt.show()